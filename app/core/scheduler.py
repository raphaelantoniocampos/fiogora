                ...
                if job.user_id:
                    ...
                    if user_creds:
                        ...
                        if global_settings is None:
                            ...
                            continue
                        # Global settings only provide URLs; usernames/passwords must come from user credentials or job metadata.
                        ...
                        # Without usernames/passwords we cannot proceed
                        logger.error(f"User credentials missing for job {job.id}; marking FAILED.")
                        await repo.update_job_status(
                            job.id,
                            SyncStatus.FAILED,
                            "User credentials not found; global settings do not contain usernames/passwords",
                        )
                        continue
                else:
        ...
        async with async_session_factory() as session:
            ...
            for job in ready_jobs:
                ...
                if job.user_id:
                    ...
                    # No user_id in job (legacy job), fall back to global settings for URLs only.
                    global_settings = await repo.get_global_settings()
                    if global_settings is None:
                        logger.error(
                            f"Global settings not found for job {job.id}; marking FAILED."
                    ...
                    ahgora_url = global_settings.ahgora_url
                    ahgora_company = global_settings.ahgora_company
                    # Cannot retrieve usernames/passwords from global settings — fail
                    logger.error(f"User credentials missing for legacy job {job.id}; marking FAILED.")
                    await repo.update_job_status(
                        job.id,
                        SyncStatus.FAILED,
                        "User credentials not found; global settings only contain URLs",
                    )
                    continue
