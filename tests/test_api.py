...
def test_run_sync_job(mock_service_class, mock_decrypt, client):
    ...
    mock_service.repo.get_user_by_username = AsyncMock(
        return_value=MagicMock(id=uuid4())
    )
    mock_service.repo.get_user_credentials = AsyncMock(
        return_value={
...
@patch("app.api.endpoints.transport_crypto.decrypt")
@patch("app.api.endpoints.SyncService")
def test_run_sync_job_missing_credentials_fails(
    mock_service_class, mock_decrypt, client
):
    mock_decrypt.side_effect = lambda x: x
    mock_service = mock_service_class.return_value
    mock_service.repo.get_user_by_username = AsyncMock(
